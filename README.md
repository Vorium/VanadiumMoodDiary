...
> **🚧 v0.32.0+137 (2026-08-13 R110 round 7b-3)**: B1-3 睡眠规律分改用 Mardia 环形统计 (跨午夜不再误判, 回归 10/10) + FS-2 daily_tracking 子树隔离 (8+ 流 watch 下沉到小节/卡片, 一次 tick 不再整页重建)。**P0 12 项 + P1 25 项中 17 项已闭环**。残余 4 fail = iOS 资产占位 (设计师) + 外部项 (keystore/域名/表单/截图)。详细报告:
