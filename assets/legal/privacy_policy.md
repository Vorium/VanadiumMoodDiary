...
## 0.6 v0.30 业务暂停 (R93 阶段 2 audit-fixes, R105 更新, v1.1.0 更新)

依据 R93 阶段 2 集中修复 (2026-08-06 完成),**部分未真接业务**已用 [FeatureFlag] 守护 + UI hidden (SizeBox.shrink, 非 disabled)。**R105 (2026-08-09) 更新**: vent + mood 语音录音已启用 (R104 决策), 从暂停清单移出, 录音数据仍按 §1 / §3 描述本地加密存储。**v1.1.0 (2026-08-16) 更新**: 失联通知 / 紧急联系人 / SMS / 邮件关怀业务**已永久删除** (代码、数据表、UI 入口全部移除), 不再属于"暂停"状态。

| # | 业务 | 业务暂停原因 | FeatureFlag |
|---|------|--------------|-------------|
| 1 | 5 厂商 push (米/华/OPPO/vivo/魅族) | 5 厂商 SDK 接入未真接 (1-2 月审核), iOS/Android 上架 blocker | `FeatureFlags.fiveVendorPushEnabled` |
| 2 | PHQ-9 / GAD-7 量表 (en / zh_Hant) | 16 题 + 严重度 + 危机电话翻译不完整 (法律责任), i18n 走 fallback key | `FeatureFlags.phqGad7I18nEnabled` |
| 3 | Android BootReceiver | v0.28 WorkManager 完善前 (设备重启后触发可能 crash) | `FeatureFlags.bootReceiverEnabled` |

**用户影响**: 除已启用的录音外, 上述暂停功能在设置页 / 量表中心入口全部 hidden。业务真接后翻 flag = 立即恢复。已删除业务 (失联通知 / 紧急联系人 / SMS / 邮件) 无恢复计划。

**数据保护**: 录音 (已启用) 按 §1 / §3 描述本地加密存储。

> 详见 `docs/CHANGELOG.md` `[0.30.0]` R93 entry + `lib/core/data/feature_flags.dart` 4 项 flag 集中定义 (v1.0.0+147 起永久完全免费)。
...
- 用户**主动**使用"导出 JSON 备份"功能(数据保存在用户剪贴板或用户指定位置)。导出范围:用药记录、打卡记录、评估报告历史、情绪日记、**树洞文字**、日常追踪记录(睡眠/体重等);**树洞录音文件不导出**(跨设备路径失效,仅 metadata 保留)
- 用户**主动**使用"分享"功能(数据通过用户选择的 App 分享)
- 法律法规要求(如公安 / 检察机关依法调取)

**本 App 不向任何第三方(包括任何联系人)发送通知、短信或邮件。** 提醒仅通过设备本地通知系统向用户本人展示。
...
| 撤回同意 | 在「设置 → 法律与隐私」页随时撤回"树洞 / 趋势分析"2 项可选功能;撤回后该功能立即停用,**数据不删除**(可重新开启),撤回时刻记录到本地供审计 |

> **v0.27 round 67 (Sprint 1) 真正生效**: 撤回同意 R67 起真正生效 — 树洞 (ConsentKind.vent) 撤回后 VentRepository.add 拒绝写入; 趋势分析 (ConsentKind.analytics) 撤回后 trend_page 显示"已撤回"占位。R67 前只更新 SharedPreferences 状态, 业务层 0 拦截 (PIPL §14 严重违反, spzh 视角 P0-6 标记)。v1.1.0 起失联通知 (ConsentKind.safety) 已随业务删除。
...
- **未加密部分**:打卡记录、用药、评估、情绪等非敏感字段(数据库整体 SQLCipher 加密已足够)
...
- 树洞 / 健康数据**永不上传**到任何服务器
- 本 App 不向任何第三方发送通知,无任何跨境 PII 传输场景

## 12. 数据导出"单独同意" (PIPL §13)
...
| v1.1.0 | 2026-08-16 | 情绪优先重构 | 失联通知/紧急联系人/SMS/邮件业务永久删除: 删 §0.5 紧急联系人告知整段; §0.6 表删 2 行 (失联 SMS / EmailService) 并改写为"已删除"状态; §1 表删紧急联系人行; §3 共享删紧急联系人并明确"不向任何第三方发送通知"; §4 撤回同意 3→2 项; §5 未加密部分删联系人; §11 跨境删未来规划表述; §12 改"数据导出单独同意" |
