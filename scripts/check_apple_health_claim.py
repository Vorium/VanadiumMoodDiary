...
#   iOS entitlement, 修正 R128d 阶段 5 守门员规则。
#
# 规则:
...
if health_kit_stub.exists():
    ...
    # 修正 1: stub 不应 import health_kit / health 包 (NoOp 阶段 0 依赖)
    for pattern in [
        r"import\s+'package:health_kit/",
        r"import\s+'package:health/",
    ]:
        if re.search(pattern, content):
            FAILURES.append(
                ...
                f'→ Apple 5.1.3 拒审 (修正 5-6 月后真接时同步加 pub 依赖 + iOS entitlement)'
            )
            break
    # 修正 2: stub 应含 NoOp 默认实现 (防未来有人改掉 NoOp 但没真接 SDK)
    ...
    # 修正 3: stub 应含 HealthKitService facade (跟 R124 5 厂商 push facade 同模式)
    ...
    FAILURES.append(
        ...
        f'修正: 写 stub (5-10L NoOp + abstract + facade), commit 后本守门员绿'
    )

# 规则 5b: FeatureFlags.healthKitEnabled 默认 false (修正 5-6 月后真接时翻 true)
...
if feature_flags.exists():
    ...
    if '_prodHealthKitEnabled = false' not in content:
        FAILURES.append(
            ...
            f'修正: 加 _prodHealthKitEnabled 静态字段 + healthKitEnabled getter + setHealthKitEnabledForTest 方法'
        )
...
if FAILURES:
    ...
    print('R128c 阶段 1 stub 占位声明: 修正 stub 文件 / flag 默认值 / NoOp 默认实现 (5-10L)。')
    sys.exit(1)
else:
