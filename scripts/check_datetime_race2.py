...
# v1.1.0 R113 (P2-20): 加 main() + exit-code 语义 (之前 report-only CI 假绿)。
import os
import re
import sys
...
def main() -> int:
    ...
    if races:
        print('=' * 60)
        print('[check_datetime_race2.py] FAIL — 同函数体多次 DateTime.now() 跨 midnight 日期漂移')
        print('修复: 函数入口 final now = DateTime.now(); 一次, 复用 now')
        return 1
    print('[check_datetime_race2.py] OK — 0 真可疑 race')
    return 0
