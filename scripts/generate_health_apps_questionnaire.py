...
# -*- coding: utf-8 -*-
"""
v0.30 R108: Google Play Health Apps Questionnaire 模板生成器

背景:
- R107 googleplay 报告 P0: Health Apps Questionnaire 0% (4 大块全空)
- 4 大块: Mental Health / Clinical Claims / Medical Device / Stigma & Equity
- R108 脚本化: 自动从 PHQ-9/GAD-7/medication/mood 业务生成声明文本
...
输出:
  build/health_apps_questionnaire.json  (结构化, 含 4 大块)
...
HEALTH_APPS_BLOCKS = {
    "1_mental_health_disclosure": {
        "title": "1. Mental Health / Behavioral Health Disclosure",
        "summary": "Does your app focus on mental health, behavioral health, or emotional well-being?",
    ...
    "3_medical_device": {
        ...
        "key_phrases": [
            ...
            "Wellness and self-management tool only",
        ],
    },
    "4_stigma_equity": {
        "title": "4. Stigma, Health Equity & Sensitive Populations",
        "summary": "Does your app address stigma, health equity, or target sensitive populations (children, elderly, LGBTQ+, etc.)?",
        "chroniccare_answer": "Addresses mental health stigma; targets general adult population",
        "disclosure": [
        ...
        "key_phrases": [
            "Helps reduce mental health stigma through private self-management",
            "Targets general adult population (18+), no sensitive population targeted",
            "No discriminatory, harmful, or stigmatizing content",
            "Multi-language support (zh / en / zh-Hant) for health equity",
...
def build_block_responses() -> dict:
    ...
    return {
        ...
        "blocks": HEALTH_APPS_BLOCKS,
    }


def render_markdown(responses: dict) -> str:
    """render 人类可读 Markdown"""
    md = f"""# Google Play Health Apps Questionnaire (R108 自动生成)
...
- [ ] Block 1 Mental Health Disclosure 已填 (答案: Yes, 4 段 disclosure)
...
- [ ] Block 4 Stigma & Health Equity 已填 (答案: 减少 stigma, 通用成年用户, 6 段 disclosure)
- [ ] Save + Submit app for review

## 关联文档

- `docs/audit/2026-08-10-cleanup/06-googleplay.md` §2.4 (Health Apps Questionnaire 详解)
- `assets/legal/medical_disclaimer.md` (R67 已加, Play Console 引用)
- `scripts/generate_data_safety_form.py` (R72, 配套 Data Safety Form)

---

> R108 简化: 4 大块结构化 + 自动从业务生成 disclosure, 避免人工漏填 / 跟隐私政策漂移。
"""
    return md
...
def main() -> None:
    ...
    print("=" * 60)
    print("Google Play Health Apps Questionnaire 模板生成器 (R108)")
    print("=" * 60)
    print()
    ...
    # JSON
    json_path = out_dir / "health_apps_questionnaire.json"
    ...
    # Markdown
    md_path = out_dir / "health_apps_questionnaire.md"
