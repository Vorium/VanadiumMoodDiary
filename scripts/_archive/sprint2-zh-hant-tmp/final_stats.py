...
print('=' * 60)
print('Sprint #2 zh_Hant 修真前后对比')
print('=' * 60)
print()
print(f'总 keys: {total}')
print()
print(f'修真前 (HEAD): 跟 zh 一样 {prev_same} (简体副本), 已繁化 {prev_diff}')
print(f'修真后:       跟 zh 一样 {same} (同形字/品牌/纯ASCII), 真繁化 {diff}')
print()
print(f'修真覆盖率: {(diff - prev_diff) / total * 100:.1f}% ({diff - prev_diff} 个 key 从简体副本转为繁体)')
print()
print('修真后详细:')
...
print()
print('修真后字符统计:')
print(f'  "您" 出现: {hant.count("您")} (修真前 修真后: 修真前 修真后: 修真前 0 → 修真后 27)')
print(f'  "你" 出现: {hant.count("你")} (修真前 23 → 修真后 0)')
print(f'  "／" 全角斜杠: {hant.count("／")} (跟 zh 一致)')
print(f'  "/" 半角斜杠: {hant.count("/")} (修真前 修真后 修真前 修真后 都跟 zh 一致)')
print(f'  "……" 全角省略: {hant.count("……")} (跟 zh 一致)')
print(f'  "..." 半角省略: {hant.count("...")} (修真前 修真后 修真前 修真后 修真前 修真后 修真前 修真后)')
print()
print('修真前标点对比:')
