...
// 层 (VentRepository / trend_page) 0 拦截 → PIPL §14 严重违反
// (spzh 视角 C-P0-6)。R67 修复路径: 业务层在 entry point 检查同意状态, 撤回
// 后该功能**真正停用** (而非只更新 SharedPreferences)。
// 1.1.0 round 4b: 原 CareEngine 消费者随外联服务整摘, 现消费者 =
// VentRepository (vent 撤回) + trend_page (analytics 撤回)。
//
// 架构原因: VentRepositoryImpl 在 data / domain 层, **不能 import
...
// 默认实现, vent_repository_provider 通过构造函数注入。
//
// 用法 (data 层):
// ```dart
