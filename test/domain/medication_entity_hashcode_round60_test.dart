...
//   element-based → 修真后应改用 element-based.
//
// 验证 (修真前): 通过 Dart 官方文档 (api.dart.dev Object.hashAll):
...
// 修真撤回. 但保留 6 个 test case 作契约回归测试, 防止未来
...
void main() {
  ...
  group('MedicationEntity == / hashCode 契约', () {
    ...
    test('相同 times 但不同 List instance → == 成立 + hashCode 一致 (修真)', () {
      // 修真前: == 成立但 hashCode 不同 (因 Object.hashAll identity)
      // 修真后: 两者都一致
      ...
      expect(a, equals(b),
          reason: '修真前 == 已成立 (== 用 _listEq element 比较)');
      expect(a.hashCode, b.hashCode,
          reason: '修真后 hashCode 必须 element-based, 跟 == 一致');
    });
    ...
    test('修真核心: Set 查找 — 修真后能 round-trip', () {
      // 修真前 bug: 2 个 == 的 med, Set 查不到对方
      // 修真后: 修真前已能 (用 _listEq 修真 ==), 但 hashCode 不同
