...
//   Apple 真实硬性要求: 1024×1024 + 无 alpha + PNG, 无最小文件体积限制
//   (50KB/200KB 为项目内误传的伪要求; 扁平设计 icon 天然 <50KB 且合规)。
//   本测试阈值 ≥10KB 只做占位防御 (68B/1×1 假图会 <2KB), 不做设计质量门。
...
void main() {
  group('R108 Fix #10: AppIcon 占位防御 (1024 ≥ 10KB, 小尺寸 ≥ 200B)', () {
    const iconDir = 'ios/Runner/Assets.xcassets/AppIcon.appiconset';

    test('Icon-App-1024x1024@1x.png ≥ 10KB (App Store 上传必看)', () {
      final file = File('$iconDir/Icon-App-1024x1024@1x.png');
      expect(file.existsSync(), isTrue);
      final size = file.lengthSync();
      expect(
        size,
        greaterThanOrEqualTo(10 * 1024),
        reason: '1024×1024 icon 太小 ($size bytes), 疑似 1×1 放大假图, 应 ≥ 10KB。'
    ...
    test('AppIcon.appiconset/Contents.json 引用所有尺寸', () {
      ...
      expect(
        content.contains('1024x1024'),
        isTrue,
        reason: 'Contents.json 应引用 1024×1024 (App Store 主尺寸)',
      );
      expect(
        content.contains('60x60'),
        isTrue,
