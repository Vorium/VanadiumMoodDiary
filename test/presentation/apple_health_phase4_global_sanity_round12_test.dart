...
// Global sanity test — feature follow 综合断言
//
// 验证 Phase 4 Task 4.1 改完, feature 都跟新 token 适配:
// 1. trend / mood / mood_list / vent / assessment / settings / daily_tracking
//    / home 全部 PageScaffold 渲染 OK (smoke render)
// 2. Divider thickness 0.5 已经被批量加到 feature 内的 11+ 个文件
...
//
// 1.1.0 round 4 (emotion-first refactor): contact feature 整摘 (目录已删),
// 9 feature → 8 feature。
//
// 注意: 本测试用 grep-based 验证 (test 期间直接读源码), 因为 feature 的
...
void main() {
  group('8 feature source-level follow', () {
    ...
    test(
      '8 feature 至少 1 个 .dart 文件存在 (smoke)',
      () {
    ...
    test(
      ...
      () {
        // 检查 feature 内: 任何 Divider(height: 1) 都不再无 thickness
        for (final d in featureDirs) {
    ...
    test(
      '8 feature 不再使用 ElevatedButton / OutlinedButton (已统一 PrimaryButton)',
      () {
        for (final d in featureDirs) {
          for (final f in dartFiles(d)) {
            final content = readFile(f);
