...
void main() {
  group('R121 P1-2 — vent_list_page god class split', () {
    ...
    test('main + entry cell 文件双存在', () {
      expect(File(mainPath).existsSync(), isTrue, reason: mainPath);
      expect(File(cellPath).existsSync(), isTrue, reason: cellPath);
    });

    test('主壳 < 500L (god-class size guard, R121 拆后 424L)', () {
      final lines = File(mainPath).readAsLinesSync().length;
      expect(
        lines,
        lessThan(500),
        reason:
            'vent_list_page.dart 主壳应保持精简 (R121 P1-2 拆后 424L), '
            '回归到 684+L 表示 _EntryCell / _VentHintHelper 被回填',
      );
    });
    ...
    test('主壳不再含 _EntryCell / _VentHintHelper', () {
      ...
      expect(main, contains('VentEntryCell'),
          reason: '主壳调 VentEntryCell (公开 widget)');
